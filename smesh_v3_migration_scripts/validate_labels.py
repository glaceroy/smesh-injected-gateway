...
def validate_label_exists(namespace, labels):
    ...
    if output.returncode != 0:
        logger.error(
            f"Failed to read namespace '{namespace}' labels: {output.stderr}"
        )
        sys.exit(1)
    ...
    for label in labels:
        if label in labels:
            logger.info(f"Label '{label}' exists on namespace '{namespace}'.")
        else:
            logger.error(f"Label '{label}' does NOT exist on namespace '{namespace}'.")
...
def main():
    ...
    for member in members_list:
        if check_namespace_exists(member):
            if apply_labels(member, labels):
                validate_label_exists(member, labels)
